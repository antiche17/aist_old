...
def test_finance(order_app):
    ...
    #with allure.step("25. Поле Синхронизация 1С успешно"): Идет отбивка ошибки про дублирование
    ...
    with allure.step("25. Поле Поставщик"):
        check.equal(order_app["is_supplier_mod"], "Бодрус - ВТБ RUR", "❌ ФР: Пустое поле Поставщик")

    with allure.step("25. Поле Покупатель"):
        check.equal(order_app["is_buyer_mod"], "ВЕСПА (Vespa Co.Ltd)", "❌ ФР: Пустое поле Покупатель")

    with allure.step("25. Поле Клиент"):
        check.equal(order_app["is_client_mod"], "ВЕСПА (Vespa Co.Ltd)", "❌ ФР: Пустое поле Клиент")

    with allure.step("25. Поле Примечание"):
        check.equal(order_app["is_note_mod"], "Примечание для ИС", "❌ ФР: Пустое поле")
        # Пустые значения в форме Создания ИС

    steps_to_check_mod = [
        ("26. Номер после редактирования", ["is_number_mod"]),
        ("27. Дата с/ф после редактирования", ["is_date_sf_mod"]),
        ("28. Океан. к/с после редактирования", ["is_ocean_vessel_mod"]),
        ("29. Фидер. к/с после редактирования", ["is_feeder_vessel_mod"]),
        ("30. Океан. судно после редактирования", ["is_ocean_ship_mod"]),
        ("31. Фидер. судно после редактирования", ["is_feeder_ship_mod"]),
        ("32. Прибытие после редактирования", ["is_arrival_mod"]),
        ("33. Отгрузка после редактирования", ["is_shipment_mod"]),
        ("34. Усл. погрузки после редактирования", ["is_loading_conditions_mod"]),
        ("35. Усл. назначения после редактирования", ["is_destination_conditions_mod"]),
        ("36. Терминал после редактирования", ["is_terminal_mod"]),
        ("37. Океан. линия после редактирования", ["is_ocean_line_mod"]),
        ("38. Фидер. линия после редактирования", ["is_feeder_line_mod"]),
        ("39. Место погрузки после редактирования", ["is_loading_location_mod"]),
        ("40. Место назначения после редактирования", ["is_destination_mod"]),
        ("41. Дата возврата после редактирования", ["is_return_date_mod"]),
        ("42. Дата вывоза после редактирования", ["is_pickup_date_mod"]),
        ("43. ГТД после редактирования", ["is_gtd_mod"]),
    ]
