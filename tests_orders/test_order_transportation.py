...
# 7–15: Проверка удаления записей
    deleted_entries = {
        "sea_type_freight": ("Тип груза", "17. Пустой Тип груза"),
        "sea_class_freight": ("Класс груза", "18. Пустой Класс груза"),
        "sea_download_method": ("Способ загрузки", "19. Пустой Способ загрузки"),
        "sea_ref_freight": ("Референс груза", "20. Пустой Референс груза"),
        "sea_booking_reference": ("Букинг референс", "21. Пустой Тип груза"),
        "sea_ocean_line": ("Океанская линия", "22. Пустое поле Океанская линия"),
        "sea_ocean_konos": ("Океанская коносамент", "23. Пустой Океанская коносамент"),
        "sea_feeder_line": ("Фидерная линия", "24. Пустой Фидерная линия"),
        "sea_feeder_konos": ("Фидерн. коносамент", "25. Пустой Фидерн. коносамент"),

        'auto_type_freight': ("Тип груза", "26. Пустой Тип груза"),
        'auto_class_freight': ("Класс груза", "27. Пустой Класс груза"),
        'auto_download_method': ("Способ загрузки", "28. Пустой Способ загрузки"),
        'auto_ref_freight': ("Референс груза", "29. Пустой Референс груза"),
        'auto_carrier': ("Перевозчик", "30. Пустой Перевозчик"),
        'auto_cmr': ("Номер CMR", "31. Пустой Номер CMR"),
        'auto_cmr_por': ("Номер CMR прож.", "32. Номер CMR порож."),
    }

    for field_name, (label, step_desc) in deleted_entries.items():
        with allure.step(step_desc):
            value = order_app.get(field_name)
            check.is_false(value and value != "[нет данных]", f"❌ ФР: Поле {label} не пустое")